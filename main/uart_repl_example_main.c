...
//ขั้นตอนที่ 3 สร้างฟังก์ชันให้ led
static int led_control(int argc, char **argv)
{
    // arg ย่อมาจาก argument คือ parameter ที่ผู้เรียกส่งผ่านมาทางวงเล็บของฟังก์ชัน
    // int argc คือ จำนวนของ parameteer เป็น integer
    // char **argv ของ parameter เป็น char* หรือ string
...
void app_main(void)
{
    ...
    // ขั้นตอบการเพิ่ม command line #1 สร้าง struct ของ command line
    // ประกอบด้วย command,ขอความช่วยเหลือ และ call back function
...
// ขั้นตอนที่ 2 ลงทะเบียน led
    // ขั้นตอบการเพิ่ม command line #1 สร้าง struct ของ command line
    // ประกอบด้วย api esp_console_cmd_register(&)
